...
const API_END_POINT="https://desi-zaika.onrender.com/api/v1/restaurant";
...
export const useRestaurantStore=create<RestaurantState>()(persist((set,get)=>({
    ...
    createRestaurant:async(formData:FormData)=>{
        try{
            ...
            if(response.data.success){
                toast.success(response.data.message || "succeed");
                set({loading:false});
            }
        }
        catch(error:any){
    ...
    updateRestaurant:async(formData:FormData)=>{
        ...
        updateMenuToRestaurant:(updateMenu:any)=>{
            set((state:any)=>{
                if(state.restaurant){
                    const updatedMenuList=state.restaurant.menus.map((menu:any)=> menu._id===updateMenu._id?updateMenu:menu)
                    return {
                        restaurant:{
                        ...state.restaurant,
